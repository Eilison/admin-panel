...
`Eilison\LaAdmin\Widgets\Box`用来生成box组件：
...
`Eilison\LaAdmin\Widgets\Collapse`类用来生成折叠插件：
```php
...
`Eilison\LaAdmin\Widgets\Form`类用来快速构建表单：
...
`Eilison\LaAdmin\Widgets\InfoBox`类用来生成信息展示块：
...
`Eilison\LaAdmin\Widgets\Tab`类用来生成选项卡组件：
...
`Eilison\LaAdmin\Widgets\Table`类用来生成表格：
