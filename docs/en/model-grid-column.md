...
The `Eilison\LaAdmin\Grid\Column` object has a built-in `display()` method to handle the value of the current column through the incoming callback function:
```php
...
use Eilison\LaAdmin\Grid\Displayers\AbstractDisplayer;
